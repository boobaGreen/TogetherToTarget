-- Crea le tabelle per il sistema di matching e gruppi
-- Da eseguire DOPO aver aggiornato la tabella categories
...
-- 6. Abilita RLS su tutte le nuove tabelle
...
-- 8. RLS Policies per groups (tutti possono vedere gruppi attivi per matching)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'groups' AND policyname = 'Users can view active groups') THEN
    CREATE POLICY "Users can view active groups" ON groups
      FOR SELECT USING (status IN ('forming', 'active'));
  END IF;
END $$;

-- Verifica prima se la colonna created_by esiste, altrimenti usa una policy più semplice
DO $$
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'groups' AND policyname = 'Group creators can update their groups') THEN
    ...
    END IF;
  ELSE
    -- Se created_by non esiste, permettiamo a tutti di aggiornare per ora
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'groups' AND policyname = 'Users can update groups') THEN
...
-- 12. Triggers per updated_at (richiede la funzione già creata)
...
BEGIN
  ...
  -- Trigger per groups (controlla se esiste già)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'update_groups_updated_at'
  ) THEN
...
BEGIN
  RETURN (
    SELECT COALESCE(up.is_available_for_matching, false)
