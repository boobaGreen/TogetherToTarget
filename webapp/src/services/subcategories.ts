...
export class SubcategoriesService {
  ...
  static async getSubcategoriesByCategory(
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `Errore nel caricamento delle subcategorie: ${error.message}`
        );
      }
      ...
      console.log(
        `✅ Caricate ${
          data?.length || 0
        } subcategorie per categoria ${categoryId}`
      );
      return data || [];
    } catch (error) {
  ...
  static async getSubcategoryById(
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `Errore nel caricamento della subcategoria: ${error.message}`
        );
      }
  ...
  static async getSubcategoriesByDifficulty(
    ...
    try {
      console.log(
        `🔍 Caricamento subcategorie categoria ${categoryId} con difficoltà ${difficultyLevel}...`
      );
      ...
      if (error) {
        console.error(
          "❌ Errore nel caricamento subcategorie per difficoltà:",
          error
        );
        throw new Error(
          `Errore nel caricamento delle subcategorie: ${error.message}`
      ...
      console.log(
        `✅ Caricate ${
          data?.length || 0
        } subcategorie difficoltà ${difficultyLevel}`
      );
      return data || [];
    } catch (error) {
  ...
  static async searchSubcategories(
    ...
    try {
      console.log(
        `🔍 Ricerca subcategorie: "${searchTerm}"${
          categoryId ? ` in categoria ${categoryId}` : ""
        }...`
      );
      ...
      query = query.or(
        `name_it.ilike.%${searchTerm}%,description_it.ilike.%${searchTerm}%`
      );
      ...
      if (error) {
        ...
        throw new Error(
          `Errore nella ricerca delle subcategorie: ${error.message}`
        );
      }

      console.log(
        `✅ Trovate ${data?.length || 0} subcategorie per "${searchTerm}"`
      );
      return data || [];
    } catch (error) {
  ...
  > {
    try {
      ...
      const { data, error } = await supabase.rpc(
        "count_subcategories_by_category"
      ); // Funzione creata nel database
